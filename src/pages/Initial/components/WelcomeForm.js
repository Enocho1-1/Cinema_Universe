...
export const WelcomeForm = () => {
  ...
  return (
    <section className="relative z-10 mt-[125px] min-xl:mt-[175px] flex flex-col items-center justify-center max-md:text-center">
       <h1 data-testid="heading" className=" font-teko text-white text-6xl max-[812px]:text-4xl">Unlimited movies, TV shows, and more</h1>
          <p data-testid="sub-heading-one" className="text-white mt-4 font-semibold font-sans text-xl max-[812px]:text-lg">Watch Anywhere. Cancel anytime.</p>
          <p data-testid="sub-heading-two" className="text-white mt-4 font-semibold font-sans text-xl max-[812px]:text-lg">Ready to watch? Enter your email to create or restart your membership.</p>
          <>

            <form data-testid="form" onSubmit={() => {initialSubmit(user.current.value,navigate)}} className="mt-4 flex items-center max-[600px]:flex-col">   
                ...
                <div className="w-full max-md:w-[50%] max-md:mt-4">
                  <button data-testid="get-started"  className="flex items-center px-6 py-2 ml-2 font-medium text-white bg-primary-blue hover:bg-secondary-blue font-teko rounded-lg"type="submit">
                    ...
                    <img src={arrow} className="h-6 ml-4" alt="arrow" />
                  </button>
                </div>
